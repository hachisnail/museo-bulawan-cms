...
export default function MainLayout() {
  ...
  const navItems = [
    ...
    },
    {
      name: "Acquisitions",
      path: "/intakes",
      icon: <Icons.Acquisitions />,
      show: true,
    },
    {
      name: "Accessions",
      path: "/accessions",
      icon: <Icons.Accessions />,
      show: true,
    },
    {
      name: "Inventory",
    ...
    <div className="flex h-screen bg-zinc-50 font-sans text-zinc-900 overflow-hidden">
      ...
      >
        ...
        <div className="h-14 flex items-center px-4 border-b border-zinc-900 overflow-hidden whitespace-nowrap">
          ...
          <div
            className={`flex flex-col transition-all duration-300 overflow-hidden ${isCollapsed ? "opacity-0 w-0 ml-0" : "opacity-100 w-auto ml-3"}`}
          >
        ...
        <div className="p-2 border-t border-zinc-900 space-y-0.5 overflow-hidden whitespace-nowrap">
          ...
          >
            ...
            <span
              className={`text-[13px] font-medium tracking-wide transition-all duration-300 overflow-hidden whitespace-nowrap ${isCollapsed ? "opacity-0 w-0 ml-0" : "opacity-100 w-full ml-3 text-left"}`}
            >
              Help & Info
            </span>
          </button>
          ...
          >
            <span className="text-zinc-500 group-hover:text-red-400 ml-0.5">
              <Icons.Logout />
            </span>
            <span
              className={`text-[13px] font-medium tracking-wide transition-all duration-300 overflow-hidden whitespace-nowrap ${isCollapsed ? "opacity-0 w-0 ml-0" : "opacity-100 w-full ml-3 text-left"}`}
          ...
          <div className="overflow-y-auto h-[calc(100vh-3.5rem)] divide-y divide-zinc-100">
            {notifications.length === 0 ? (
              <div className="p-8 text-center flex flex-col items-center justify-center h-full text-zinc-500">
                <Icons.Bell />
                <p className="mt-4 text-xs tracking-wider uppercase font-medium">
                  No active alerts
                </p>
                <p className="mt-1 text-[11px] font-light text-zinc-400">
                  Your archive is secure and up to date.
                </p>
              </div>
            ) : (
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      ...
                      if (notif.action_url)
                        navigateToResource(notif.action_url);
                    }}
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      <div className="flex items-center gap-2">
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${typeColors[notif.type] || "bg-zinc-400"}`}
                        ></span>
                        <h4
                          className={`text-[13px] font-bold tracking-tight ${notif.is_read ? "text-zinc-600" : "text-black"}`}
                        >
                          {notif.title}
                        </h4>
                      </div>
